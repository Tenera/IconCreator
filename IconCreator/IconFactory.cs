    ...
    public static class IconFactory
    {
        ...
        public static void CreateMultiImageIcon(string sourceImage, string destinationImage)
        {
            using (var source = (Bitmap) Image.FromFile(sourceImage))
            {
                ...
                if (source.Width < 256)
                {
                    throw new InvalidOperationException("Dimensions of the source image must be at least 256x256");
                }
        ...
        private static void ThrowForInvalidPngs(IEnumerable<Bitmap> images)
        {
            foreach (var image in images)
            {
                ...
                if (image.RawFormat.Guid != ImageFormat.Png.Guid && image.RawFormat.Guid != MsFormatPng)
                {
